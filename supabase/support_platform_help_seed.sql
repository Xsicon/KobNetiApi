-- KobNeti platform help articles (internal staff docs for /admin/help)
-- Apply after support_w7_insights.sql (requires ops_help_articles table)
-- Idempotent: safe to re-run; updates body/title/category on conflict

insert into sominnercore.ops_help_articles (slug, title, body, category, status, sort_order)
values
(
  'getting-started',
  'Getting Started with KobNeti',
  $body$KobNeti is your operations workspace for support, engineering, people ops, and platform administration. Each product (brand) has its own data scope — switch products from the sidebar without signing out.
...
The public Knowledge Base (Support → Knowledge Base) is for customers. This Help Center (/admin/help) is for internal staff documentation only.$body$,
  'getting-started', 'published', 1
),
(
  'support-hub',
  'Support Hub Overview',
  $body$The Support Hub is your home for live chat, tickets, incidents, and the customer knowledge base.
...
Create and publish articles that appear on the public help center. Draft articles stay hidden until published.$body$,
  'support', 'published', 2
),
(
  'live-chat-tickets',
  'Live Chat & Tickets',
  $body$This guide covers day-to-day ticket and chat workflows for support agents.
...
5. SLA awareness:
Watch SLA badges on tickets. Escalate urgent items before breach when your team policy requires it.$body$,
  'support', 'published', 3
),
(
  'engineering-board',
  'Engineering Task Board',
  $body$Engineering tools help your team plan work, track milestones, and connect to GitHub repositories.
...
5. Assignees:
Assign tasks to staff members who have access to the product. Assignee lists come from your staff roster.$body$,
  'engineering', 'published', 4
),
(
  'time-tracking',
  'Time Tracking & Approvals',
  $body$Accurate time tracking feeds approvals and payroll. Clock times use your device timezone, not the account default.
...
5. Timezone note:
Each entry stores the IANA timezone from your browser (e.g. Africa/Mogadishu). Traveling staff should verify times look correct before submitting.$body$,
  'people', 'published', 5
),
(
  'payroll-basics',
  'Payroll Basics',
  $body$Payroll uses approved time data. Platform admins can run payroll exports; managers review approvals first.
...
5. Permissions:
Only users with platform admin role can execute payroll runs. Contact your administrator if you need access.$body$,
  'people', 'published', 6
),
(
  'calendar-events',
  'Calendar & Events',
  $body$The shared calendar helps teams track milestones, renewals, and operational events.
...
5. Product scope:
Calendar events are scoped to the active product tenant.$body$,
  'platform', 'published', 7
),
(
  'file-management',
  'File Management',
  $body$Upload and organize operational files — separate from ticket attachments and chat uploads.
...
5. Troubleshooting:
If upload fails with "Bucket not found", ask your platform admin to create the ops-files bucket in Supabase.$body$,
  'platform', 'published', 8
),
(
  'audit-logs',
  'Audit Logs',
  $body$Audit logs provide a tamper-evident trail of important actions across the platform.
...
5. Retention:
Retention policy depends on your Supabase and API configuration. Contact your administrator for archival rules.$body$,
  'platform', 'published', 9
),
(
  'notifications',
  'Notification Preferences',
  $body$Control which in-app alerts you receive for assignments, approvals, and escalations.
...
5. Per-user settings:
Preferences are saved per user and per product tenant.$body$,
  'platform', 'published', 10
),
(
  'product-registry',
  'Product Registry',
  $body$The Product Registry lists every brand in your KobNeti workspace and links operational metadata.
...
5. Multi-product ops:
Switch products from the sidebar — registry is the map of everything you manage.$body$,
  'platform', 'published', 11
),
(
  'asset-management',
  'Asset Management',
  $body$Track laptops, licenses, and equipment assigned to your team.
...
5. Filters:
Search by name, serial, or assignee. Filter by type and status to audit inventory.$body$,
  'platform', 'published', 12
),
(
  'insights-reports',
  'Insights & Reports',
  $body$Insights aggregates operational metrics and lets you run exportable reports.
...
5. When to use:
Use Insights for standups, weekly ops reviews, and leadership dashboards.$body$,
  'platform', 'published', 13
),
(
  'internal-chat',
  'Internal Team Chat',
  $body$Internal Chat is a lightweight team channel for staff coordination — not customer-facing.
...
Customer issues should still be tracked in Support Hub for SLA and history.$body$,
  'platform', 'published', 14
)
